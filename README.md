...
5. На последней задаче приложение проверяет пропуски: при необходимости ведёт к первой неоценённой задаче, а после заполнения всей очереди показывает кнопку «Завершить оценку».
